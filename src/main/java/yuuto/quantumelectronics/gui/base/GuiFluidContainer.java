...
public abstract class GuiFluidContainer extends GuiScreen{
    ...
    private void func_146977_a(Slot p_146977_1_)
    {
        ...
        if (!flag1)
        {
            ...
            GL11.glEnable(GL11.GL_DEPTH_TEST);
            if(itemstack != null && itemstack.getItem() == ModItems.FLUID_DUMMY){
            	FluidStack fluid = ((IFluidContainerItem)itemstack.getItem()).getFluid(itemstack);
            	Block block = fluid.getFluid().getBlock();
            	if(block != null){
            		ItemStack fluidItem = new ItemStack(block);
            		//fluidItem.stackSize = fluid.amount;
            		itemRender.renderItemAndEffectIntoGUI(this.fontRendererObj, this.mc.getTextureManager(), fluidItem, i, j);
                	itemRender.renderItemOverlayIntoGUI(this.fontRendererObj, this.mc.getTextureManager(), fluidItem, i, j, s);
            	}else{
